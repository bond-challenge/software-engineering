'''
Write an interface for a data structure that can be used for below options
...
import AverageElementInterface


# create a class to implement the inteface
class LastAvgElement(AverageElementInterface.MyAvgInterface):
    var_List = []
...
while val_input == 0:
    ...
    if val_input == '1':
        ...
        val_YN = input('Would you like to Continue (Type Y OR N): ')
        if val_YN == 'Y' or val_YN == 'y':
            val_input = 0
        else:
            break
        ...
        if len(obj_LastAvgElement.var_List) == 0:
            print('No item has yet been added to data structure. Kindly add elements first')
            val_YN = input('Would you like to Add (Type Y OR N): ')
            if val_YN == 'Y' or val_YN == 'y':
                obj_LastAvgElement.AddElement()
            else:
                val_input = 0
